...
The approved design is selective protocol reuse plus an injectable transport. Import `benchweave.devices.fnirsi.dps150` from the existing BenchWeave distribution (Python >=3.13). No new runtime dependencies or separate distribution are introduced; use the repository's uv.lock.

Framing, checksum, float byte order and field layouts are ported from [cho45/fnirsi-dps-150 at 6107bd34531aa52692d757e3b82d9573750311ac](https://github.com/cho45/fnirsi-dps-150/blob/6107bd34531aa52692d757e3b82d9573750311ac/dps-150.js). Its MIT copyright/permission notice is retained in `src/benchweave/devices/fnirsi/dps150/LICENSE`. No browser UI, serial reader loop, sleeps, session startup or automatic state management are reused.
...
```python
from benchweave.devices.fnirsi.dps150 import Client, GetPayload
...
All fixtures in `tests/devices/fnirsi/dps150/test_protocol.py` are explicitly synthetic. Literal golden frames check requests/responses independently of the production encoder. The ALL fixture is assembled from documented byte offsets, not captured hardware.
...
Run `uv run --no-sync pytest tests/devices/fnirsi/dps150/test_protocol.py`; existing test discovery, ruff and mypy configuration includes this package. No separate dependency lock changes are needed.
