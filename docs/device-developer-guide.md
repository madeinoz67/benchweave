...
**Delivery ceiling, with its derivation.** `next_event` returns one event per call, events flow only inside the poll rhythm, and the poll slice is floored at the 10 ms bench poll cadence — so the shared budget is approximately **100 events/s across all live subscriptions**. A device with N variables at R Hz demands N×R events/s; a device whose product approaches the budget belongs on the capture or dataset lane, streaming a decimated signal at most. `min_interval_ms` is a maximum emission rate, not a guarantee of hardware sample rate — do not advertise streaming as a substitute for acquisition.
