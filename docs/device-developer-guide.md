...
**Corpus gate (issue #176):** the capture step branch and its capture allow-rule branch live in the open execution dev head (`standards/execution/0.2.0-dev/`) and are admitted by a `CorpusResolution.DEV_HEAD`-composed gateway; the promoted active execution release carries them from the train's promotion event onward. Until then the frozen-literal active corpus refuses a capture step at the procedure schema — author against the head, and the dev-proof lane (`check_execution.py --corpus standards/execution/0.2.0-dev`) proves the documents.
