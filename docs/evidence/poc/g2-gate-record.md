...
| D4 (legacy free-form event evidence) | **CLOSED (interface-errata slice, 2026-09-17, `a1a99c4`)** — every event kind emits the closed doc-ref (binding document / bench configuration / change target), every served event validates against the vendored `event` def; the deviation pin flipped to all-kinds conformance. Was: ACCEPTED G2 DEVIATION, resolution deferred to the dedicated interface-errata slice; pinned meanwhile by `test_interface_parity.py::test_event_evidence_shape_deviation` | `decisions/deferred-deviations.md` (D4 section) |
| D14, details half (six-key error `details` envelope) | **CLOSED (interface-errata slice, 2026-09-17, `41d547a`)** — every failure envelope serves the closed six-key object with typed findings, watermarks and `current_revision` populated where context supplies them; the rendered error validates against the vendored `$defs/error`. Was: ACCEPTED G2 DEVIATION, same interface-errata slice as D4; the correlation_id half is **CLOSED** by WP09 Task 1 (`3ebfcb5` — every failure envelope mints a real id) | `decisions/deferred-deviations.md` (D14-details section) |
