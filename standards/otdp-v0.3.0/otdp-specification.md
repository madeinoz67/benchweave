...
`required_features` contains `otdp.core/0.3.0`, plus `otdp.adapter/1.1` for adapters and `otdp.passive_can/0.3.0` for declarative CAN. Class integrations additionally require otdp.profile_actions/1.0.0, otdp.measurement/0.3.0 and their exact profile IDs. An unsupported feature or version is an admission failure. Optional namespaced `x-vendor-name` fields may be ignored at schema extension points; required semantics MUST NOT depend on them.
