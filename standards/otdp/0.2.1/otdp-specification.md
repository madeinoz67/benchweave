...
`custom` may carry one optional `provider` object pinning a transport-provider contract: the feature the integration requires, and the `{id, path, sha256}` triple naming the reviewed document inside the admitted bundle (extension-contract §6, transport-providers.md). When present, `connection_key` resolves to a connection backed by that admitted contract and the adapter's transfers use the contract's transaction grammar (§8.1). When absent, the integration is honestly incomplete for that device: nothing about a connection key implies a provider, and the core never falls back to unrestricted I/O.
...
The initial generic HostServices has no `custom` transaction kind. An integration declaring transport custom must reference a separately documented and admitted host-service extension; since 0.2.1 that extension is a pinned transport-provider contract (§6.4, extension-contract §6, transport-providers.md). Its `transaction_grammar` extends this table for that provider only: the host validates provider transactions against the admitted contract's grammar, never against this generic table. Provider transactions are `transfer` calls on the commissioned connection — they reject unspecified fields, carry no host/path/credential fields, inherit transfer's deadline, cancellation and evidence enforcement, and remain governed by the `scoped_transport` permission. An agent cannot mark a custom integration complete using these generic services alone. The core never falls back to unrestricted I/O.
...
**reference-hid-meter:** A USB-HID-class power meter behind a pinned provider contract, demonstrating the `custom` transport's declaration. The descriptor pins `reference-provider.json` (feature `otdp.transport.reference_hid/1.0.0`); the contract's grammar — `hid_output_report` and `hid_input_report` — is the entire protocol surface, exercised offline by exact-dict transfer scripting rather than wire vectors. Identity is adapter-sourced. `bus_power` is a read-only measurement in watts. No wire protocol, host implementation or hardware qualification is defined by this example; it is not an admitted host provider.
