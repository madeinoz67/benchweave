...
## 6. Transport coverage and provider contracts

Class contracts are transport-independent. The existing scoped LAN/USBTMC/serial/CAN/I²C/SPI primitives are retained. A device requiring GPIB, USB-HID, arbitrary USB bulk or a vendor SDK declares it through a pinned transport-provider contract instead of remaining outside the system. API 1.1 does not grant direct unrestricted SDK/filesystem/network access as a shortcut. The relevant class may be fully specified while a particular device's transport integration remains unsupported.

A descriptor whose transport is `custom` may carry one optional `provider` object: a `feature_id` in the required_features pattern that must also appear in `required_features`, plus the `{id, path, sha256}` triple in this contract's reference shape — package-relative, digest-pinned, resolved inside the admitted bundle. The pinned document is a transport-provider contract validated by `otdp-transport-provider.schema.json` (transport-providers.md): a transaction grammar extending the specification's §8.1 transfer table for that provider only, a closed `security_scope`, reviewed host requirements and an approval block in the commissioning evidence shape.

Two review tiers, both required. This mechanism — the schema and these rules — is corpus content under standards governance. Each provider contract instance is admitted by the host operator as a commissioning-class act: the document's approval block records the review, and the host's admission record grants authority. A descriptor pinning a contract the host has not admitted is an admission failure; device-supplied metadata cannot add a transport provider on its own (section 2). The pin makes the reviewed bytes deterministic; the operator's admission makes them authoritative. Without `provider`, `custom` keeps its prior meaning: the integration is honestly incomplete for that device.
