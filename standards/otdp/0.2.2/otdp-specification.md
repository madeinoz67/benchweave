...
**reference-hid-meter:** A USB-HID-class power meter behind a pinned provider contract, demonstrating the `custom` transport's declaration. The descriptor pins `reference-provider.json` (feature `otdp.transport.reference-hid/1.0.0`); the contract's grammar — `hid_output_report` and `hid_input_report` — is the entire protocol surface, exercised offline by exact-dict transfer scripting rather than wire vectors. Identity is adapter-sourced. `bus_power` is a read-only measurement in watts. No wire protocol, host implementation or hardware qualification is defined by this example; it is not an admitted host provider.

**reference-rawlink:** A `custom` transport with no `provider` object, demonstrating the honestly-incomplete state §6.4 keeps admitted. The connection key names a commissioned link whose backing is undocumented; no provider contract is declared, none may be implied, and no capability beyond `identify` is advertised. The descriptor is structurally admissible and an agent cannot mark it complete using the generic services alone. There is no wire protocol to define — that is the point of the example.
