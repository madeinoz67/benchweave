...
- D2 ``change_apply`` body — CLOSED (WP08 Task 6, interface/1.1.1 errata):
  ...
  ``contracts/interface/1.1.1/`` (vendored from ``docs/interface/1.1.1/``;
...
CONTRACTS = Path(__file__).resolve().parents[2] / "standards" / "interface/1.1.1"
...
def test_change_apply_approver_token_end_to_end(gateway: SimpleNamespace) -> None:
    """D2 errata pin (REST-only surface): the interface/1.1.1 body schema
