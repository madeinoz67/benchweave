...
def sdk_module(name: str):  # type: ignore[no-untyped-def]
    assert importlib.util.find_spec("benchweave_sdk") is not None, "SDK is not implemented"
...
def test_offline_schema_and_correlation_validation() -> None:
    validation = sdk_module("validation")
...
def test_scaffold_builds_only_in_new_directory_and_has_tests(tmp_path: Path) -> None:
    cli = sdk_module("scaffold")
    target = tmp_path / "external"
    cli.create_project(target, "example_plugin")
    ...
    for name in ("../escape", "not-a-module", "class"):
        with pytest.raises(ValueError):
            cli.create_project(tmp_path / "bad", name)
...
def test_manifest_inventory_rejects_tamper_and_escape(tmp_path: Path) -> None:
    packaging = sdk_module("packaging")
...
def test_generated_adapter_maps_host_resource_failure(
    ...
    sdk_module("scaffold").create_project(tmp_path / "plugin", "resource_failure_plugin")
    ...
    async def run() -> None:
        ...
        sdk_module("validation").validate_result(result, request)
...
def test_missing_schema_resource_fails_offline() -> None:
    ...
    registry = sdk_module("validation")._registry()
