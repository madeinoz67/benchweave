...
- **Vault routing:** every memory for this project belongs in the `benchweave` vault — never the
  default MuninnDB vault. Direct `muninn_remember`/`muninn_recall` over MCP only through the
  project-scoped server (`.mcp.json`, needs its one-time `/mcp` approval; a globally-configured
  muninndb connection is typically key-scoped to `default` and cannot address `benchweave`).
