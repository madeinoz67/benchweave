...
description: >-
  ...
  pick a side: an architecture-doctrine question, a design that returned two defensible
  shapes, or a measurement that contradicts something already landed. Independent
...
1. A **Tier-3 or doctrine fork** — a decision that changes what the product means, is
   expensive to reverse, or contradicts an existing invariant (`docs/internal/invariants.md`)
   or a closed decision in `docs/smart-test-gateway-decisions.md`.
...
`increment` skill already says. Three independent runs plus a judge is a real cost, and
...
Before any panelist runs, write to the design record (in `.claude/deep-review/`, or a
scratch file if no design record exists yet):
...
This is the same pre-registration discipline the project already holds its measurements
to. A rule written after the arguments arrive is not a rule, it is a rationalization.
...
answer: two independent runs reject both options on the table and converge on a third
shape nobody had proposed.

Run them **concurrently and blind** — no panelist sees another's output. Use different
models where you can; the independence is the point. Each panelist reads the real code,
the invariants and the decision record, and cites `file:line`. A panelist that reasons
from the brief alone is producing an opinion, and opinions are what this procedure
exists to avoid.
...
1. **Mark each claim** verified, refuted, or unverifiable — against the live code, the
   invariants and the decision record, not against plausibility. Unverifiable claims are
...
**The judge may NOT introduce a new option.** That is what panelist three was for. A
judge that invents an answer has skipped the independence the whole procedure is built
on.
...
that lost, with their strongest points intact.** A panel that ends in a summary has
failed. The losing case is the most valuable artifact six months later, when someone asks
why the project went this way — and it is what makes the decision reversible on evidence
rather than on memory. Where the outcome changes architecture, it becomes a new entry in
the decision record when it lands.

## Failure modes to watch for

- **Panelists converging because they read each other.** Run them blind or the result is
