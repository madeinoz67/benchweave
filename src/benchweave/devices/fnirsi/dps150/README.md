...
The reviewed factory is `benchweave.devices.fnirsi.dps150.adapter:create_plugin`. It wraps the bundled injectable protocol library, retaining its selective reuse of cho45's MIT framing algorithms and KochC's corroborating command evidence. See `LICENSE` and the repository's `docs/dps150-protocol.md` for source/licensing and protocol limitations. There are no additional runtime distributions to install: descriptor `dependencies` is empty; code and resources ship with BenchWeave's root `pyproject.toml`. This is not a separately released registry package.
...
This adapter fixes **KochC's empty GET payload**. cho45's zero-byte GET variant is not automatically tried. Opening sends nothing: the library deliberately excludes undocumented B0/C1/C0 initialisation and flash commands. A host-established clean, already responsive session is a commissioning prerequisite, not an implemented handshake. If that prerequisite cannot be established using separately reviewed evidence, this adapter is not hardware-ready.

The scoped bridge uses `stream_send`, then exact 4-byte header and payload-plus-checksum receives. `exact_bytes` overrides the required `lf` grammar field; no line terminator is appended or consumed. Host exact receives must either return exactly the requested bytes or raise. Unsolicited or delayed same-field responses cannot be distinguished on this uncorrelated wire protocol. The host must supply a clean session without unsolicited telemetry; mocks do not prove this physical property. A failed exchange permanently blocks the instance. No drain, retry, reconnect or replay is hidden in the adapter.
...
Run `uv run --no-sync pytest tests/devices/fnirsi/dps150`. Scalar request/response/result vectors are `adapter-vectors.json`; replayable failure cases are `adapter-failure-vectors.json` (read cases start after the mock identity exchange). Identity and additional boundary stimuli are explicit literals/mock injections in the tests. Root tests validate the locally pinned descriptor and runtime schemas without network resolution.
